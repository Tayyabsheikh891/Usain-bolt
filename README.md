# Usain-bolt
A project written on python integrated with Tkinter GUI for Detecting Usain Bolt Pose
#import libraries
import tkinter
import cv2
from tkinter import*
...
l1.place(x = 475, y = 25, width=550, height=40)
...
#Reference Image
image=cv2.imread("usainbolt.jpeg")  #Reference Image
...
global cap,checkinit
checkinit=0 #initially zero

def vout(vname):
    global cap,checkinit
    checkinit +=1 #value change to 1 when video is run
    ...
    def select_img():
        ...
        #adjust video window on new size
        ...
        try:
                ...
                #webcam/video calculated angles
        ...
        if(devp<50):    #Leniency
            ...
            if(displayp>=60):           #Success Threshold
                if(displayp>=60): # Counting success threshold
                    if(detected==False):
                        counter=counter+1
                        detected=True
                        #update counter
                        countlabel=Label(win,text=str(counter),borderwidth=2, relief="groove")
                        countlabel.place(x = 1010, y = 80, width=80, height=35)
